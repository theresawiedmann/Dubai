...
aidOA <- fread("C:/Users/wiedmann4/Dropbox/Aid and corruption/Temp/Data_Aid_OECD_AidData.csv")

# Merger with aid from GODAD (1995-2023)
#aidG <- fread("C:/Users/wiedmann4/Dropbox/Aid and corruption/Out/Data_Aid_GODAD.csv")

country_folders <- list.dirs(path = "C:/Users/wiedmann4/Dropbox/Aid and corruption/Raw data/Sandcastles",
                             full.names = TRUE, recursive = FALSE)

sandcastle <- lapply(country_folders, function(country_path) {
...
sandcastle <- type_convert(sandcastle) # convert back / also type_convert()
#sapply(sandcastle, class) #info on class 
sandcastle <- sandcastle %>%
  relocate(country, data_year, .before = 1)
#saveRDS(sandcastle, "Temp/sandcastle.rds")
...
for (i in seq_along(politicians_lower)) {
  ...
  if (length(tokens) == 1) {
    ...
    idx <- str_detect(sandcastle$SandName_clean, regex(pattern))
    ...
    idx <- str_detect(sandcastle$SandName_clean, regex(pattern_fwd)) |
      str_detect(sandcastle$SandName_clean, regex(pattern_rev))
  }
  
  sandcastle$LeadersName[idx] <- p_orig
}

#sub_merge <- sandcastle %>% filter(if_any(LeadersName, is.na)) was used to check 
#which names from the Sandcastles db did not have a LeaderName
...
#write.csv(Sand_fullnames, "C:/Users/wiedmann4/Dropbox/Aid and corruption/Temp/Sand_fullnames.csv")
...
# Merge of Sand, Leaders and aid data from OECD and AidData
SLO <- aidOA %>%
  left_join(
    Sand %>% 
      mutate(SandDummy = 1),
...
write.csv(SLO, "C:/Users/wiedmann4/Dropbox/Aid and corruption/Temp/DubaiDataOA.csv")

SLO <- SLO %>%
  filter(SandDummy == 1)

#summary(SLO)
write.csv(SLO, "C:/Users/wiedmann4/Dropbox/Aid and corruption/Temp/DubaiDataOA_matchesonly.csv")
rm(sandcastle, Missing, Sand, SLO, aidOA)
